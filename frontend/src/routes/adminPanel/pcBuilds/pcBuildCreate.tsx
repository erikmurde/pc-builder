...
import PcBuildCreateFormView from "./views/pcBuildCreateFormView";
...
const PcBuildCreate = () => {
    ...
    const validate = (values: IPcBuildCreateDTO) => {
    ...
    const onSubmit = async (values: IPcBuildCreateDTO) => {
        ...
        let response = await pcBuildService.create(values, jwtData);
        if (response) navigate('../pcBuilds');
    }

    return (
        <PcBuildCreateFormView initialValues={initialValues} selectValues={selectData} validate={validate} onSubmit={onSubmit} />
    );
}

export default PcBuildCreate;